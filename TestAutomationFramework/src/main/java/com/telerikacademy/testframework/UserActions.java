...
public class UserActions {
    ...
    public void dragAndDropByXpath(String sourceXpathKey, String targetXpathKey) throws InterruptedException {
        int defaultTimeout = Integer.parseInt(getConfigPropertyByKey("config.defaultTimeoutSeconds"));

        String sourceXpath = getLocatorValueByKey(sourceXpathKey);
        String targetXpath = getLocatorValueByKey(targetXpathKey);

        waitForElementVisibleUntilTimeout(sourceXpath, defaultTimeout);
        waitForElementVisibleUntilTimeout(targetXpath, defaultTimeout);

        WebElement sourceElement = driver.findElement(By.xpath(String.format("//span[text()=\"%s\"]", "My First Card")));
        WebElement targetElement = driver.findElement(By.xpath(String.format("//textarea[text()=\"%s\"]", "Doing")));
        ...
        try {
            ...
            Assert.fail("Drag and Drop failed: " + exception.getMessage());
        }
        Thread.sleep(3000);
    }
